...
class VectorEmbeddedFieldValueHandler implements FieldValueHandler {
  ...
  private final VectorIndexFieldDefinition fieldDefinition;
  ...
  private VectorEmbeddedFieldValueHandler(
      ...
      VectorIndexFieldDefinition fieldDefinition,
   ...
   *
   * <p>If there is a nested VectorEmbeddedDocumentsFieldDefinition, we need to keep looking for
   * embedded documents as we descend this subtree by wrapping in a {@link
   * VectorEmbeddedDocumentHandler}.
   */
  private Optional<DocumentHandler> wrappedSubDocumentHandler() {
    ...
    // For vector indexes, we don't have a hierarchical field definition structure like search
    // indexes, so we just return the wrapped sub document handler directly
